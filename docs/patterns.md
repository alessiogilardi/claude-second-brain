...
|---|---|---|
| Create-only scaffolding | `copy_if_absent` in `bootstrap/bootstrap.sh`, for `docs/` and the CI workflow | An existing destination file is never overwritten and nothing is ever deleted, so an accidental re-run is a no-op — the bootstrap must not clobber content the user has since edited. |
...
| Triple-mirrored enforcement | `EXCLUDE_PATTERN` duplicated verbatim across `bootstrap/payload/git-pre-commit` (inside the injectable block, the canonical copy for both a fresh hook and an injected one), `hooks/session-reminder.sh`, and `bootstrap/payload/workflows/second-brain.yml` | `core.hooksPath` is local git config, never cloned — a single local hook gives zero enforcement on a fresh clone or a skipped bootstrap, so the same syntactic check is repeated at three points (local hook, Stop hook, CI). Deriving the injected block from the payload keeps the mirror at three, not four. See ADR 0002. |
