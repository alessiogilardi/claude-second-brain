...
| Implementation package vs. template content | `skills/`, `hooks/`, `workflows/` (complete, non-templated, copied byte-for-byte) live at the repo root; `template/` holds only mergeable fragments and placeholders (`CLAUDE.md`, `settings.json`, `docs/`) | Keeps "things you edit as real, functioning code" separate from "things meant to be filled in or merged"; `install.ps1` maps each source package to its destination path explicitly instead of relying on identical relative paths. |
